...
export function generateUsage(
  ...
  // Convert path to command segments
  ...
  if (argsSchema && "shape" in argsSchema) {
    const shape = (argsSchema as z.ZodObject<Record<string, z.ZodTypeAny>>).shape;
    for (const [key, value] of Object.entries(shape)) {
      const isOptional = value.isOptional?.() ?? false;
      const aliasStr = aliases?.[key]
        ? ` | -${aliases[key].join(" | -")}`
        : "";

      if (isOptional) {
        parts.push(`[--${key}${aliasStr}]`);
      } else {
        parts.push(`--${key}${aliasStr} <value>`);
      }
    }
  }
